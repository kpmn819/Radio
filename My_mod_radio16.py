...
# V-16 working on debugging tune_sta so it goes to the home station and moves the dial
...
def tune_sta(sta_num):
    ...
    print('enc_val=',enc_val,' enc_prev=',enc_prev)
    # get new angle
...
def check_list(parsme):
    ...
    for i in range(0,len(parsme)):
        ...
        for x in range(i+1, len(parsme)):
            # how close together can they be hardcoded 
            if abs((parsme[i] - parsme[x])) < 35:
                good_list = False
    return good_list
