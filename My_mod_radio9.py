...
def init():
    ...
    GPIO.add_event_detect(Enc_A, GPIO.FALLING, callback=rotation_decode, bouncetime = 10) # bouncetime 2 in mSec
...
def rotation_decode(Enc_A):
    ...
    if (Switch_A == 1) and (Switch_B == 0) : # A then B ->
        ...
        if enc_prev != enc_val:
            angle = tune_needle(enc_val)
            print('New Encoder Down: ',enc_val)
            #tuner(enc_val, angle)
