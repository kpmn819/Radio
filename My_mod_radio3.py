...
def tuner(enc_val):
    ...
    for i in range(0, len(freq_list)):
        ...
        #diff = abs(freq_list[i] - 15) unnecessary code
...
def noise(vol_val):
    ...
    # call amixer to set volume
    if int(vol_val) != 100:
        if noise_prev == False:
            # start playing noise
            #pygame.mixer.music.play()
            pygame.mixer.music.unpause()
            # invert the volume to ramp up noise as you tune away
            scale_vol = (100 - int(vol_val)) / 100
            print('scale_vol ', str(scale_vol))
            #subprocess.Popen("aplay /usr/share/sounds/alsa/white_noise.wav ", shell=True)
            pygame.mixer.music.set_volume(scale_vol)
            #
        print('        Playing Noise Now: ', vol_val)    
        noise_prev = False
    else:
        noise_prev = True
        # Pause play
